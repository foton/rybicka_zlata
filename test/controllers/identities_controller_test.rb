...
class Users::IdentitiesControllerTest < ActionController::TestCase
  ...
  def test_cannot_see_identities_for_other_user_account
    ...
    post :create, params: { user_id: other_user.id, user_identity_as_contact: idnt_params }
  ...
  def test_identities_are_created_for_current_user_only
    ...
    assert_difference('User::Identity.count') do
      post :create, params: { user_id: @current_user.id, user_identity_as_contact: { email: @new_email, provider: User::Identity::LOCAL_PROVIDER } }
    end
  ...
  def test_convert_to_local_identity
    assert_difference('User::Identity.count') do
      post :create, params: { user_id: @current_user.id, user_identity_as_contact: { email: @new_email, provider: 'test' } }
    end
  ...
  def test_not_create_local_identity_without_valid_email
    assert_no_difference('User::Identity.count') do
      post :create, params: { user_id: @current_user.id, user_identity_as_contact: { email: '', provider: User::Identity::LOCAL_PROVIDER } }
    end
  ...
  def test_cannot_delete_others_identity
    ...
    assert_no_difference('User::Identity.count') do
      delete :destroy, params: { user_id: @current_user.id, id: @identity.id }
    end
