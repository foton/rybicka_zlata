...
class GroupsControllerTest < ActionController::TestCase
  ...
  def test_created
    ...
    post :create, {user_id: @current_user.id, group: grp_h}
    ...
    patch :update, {user_id: @current_user.id, id: new_group.id, group: edit_grp_hash }
  ...
  def test_not_created
    ...
    assert_not_nil assigns(:user)
    assert (assigns(:group).errors[:name].size > 0)
  ...
  def test_edit
    ...
    get :edit, {user_id: @current_user.id, id: group.id}
  ...
  def test_update
    group=Group.create!(user: @current_user, name: "My group")
    edit_grp_hash={name: group.name, connection_ids: @connections.collect {|con| con.id} }

    patch :update, {user_id: @current_user.id, id: group.id, group: edit_grp_hash }
