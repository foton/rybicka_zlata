...
class UserWishTest < ActiveSupport::TestCase
  ...
  def test_know_his_all_kind_of_wishes
     #author wishes
     assert_equal [@author_wish1.id,@author_wish2.id].sort , (@user.author_wishes.collect {|w| w.id}).sort   #I do not care if it is Wish::FromAuthor or Wish::FromDonee or Wish::FromDonor
     #donee wishes
     assert_equal [@author_wish1.id, @author_wish2.id, @mama_shared_wish.id, @tata_shared_wish.id].sort , (@user.donee_wishes.collect {|w| w.id} ).sort
     #donor wishes
